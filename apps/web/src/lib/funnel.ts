...
export function filterFunnel(
  ...
  return opportunities.filter((opportunity) => {
    const matchesQuery =
      !query ||
      [opportunity.organizationName, opportunity.domain, opportunity.nextAction?.title ?? ""].some(
        (value) => value.toLocaleLowerCase("ru").includes(query),
      );
    const matchesStage = filters.stageCode === "all" || opportunity.stageCode === filters.stageCode;
    const matchesOwner = filters.ownerId === "all" || opportunity.owner.id === filters.ownerId;
    const matchesRisk = filters.risk === "all" || opportunity.riskFlags.includes(filters.risk);
...
export function summarizeFunnel(opportunities: FunnelOpportunity[]) {
  return {
    ...
    missingNextAction: opportunities.filter(({ riskFlags }) =>
      riskFlags.includes("missing-next-action"),
    ).length,
    technicalRisk: opportunities.filter(({ riskFlags }) => riskFlags.includes("technical-risk"))
      .length,
  };
}
