import { ArrowUpRight, Check, Clock3, ExternalLink, GitBranch, GitMerge, UserPlus, X } from "lucide-react";
  ...
  return (
    <aside className="detail-panel" aria-label={`Контекст: ${task.organizationName}`}>
      <div className="detail-header">
        ...
        <button className="icon-button" type="button" onClick={onClose} aria-label="Закрыть контекст">
          <X size={18} aria-hidden="true" />
        </button>
      </div>

      <dl className="detail-definition-list">
        <div><dt>Сайт</dt><dd>{task.domain} <ExternalLink size={13} aria-hidden="true" /></dd></div>
        <div><dt>Стадия</dt><dd><span className="stage-label">{task.stageLabel}</span></dd></div>
        <div><dt>Ответственный</dt><dd>{task.ownerName}</dd></div>
      </dl>

      <section className="detail-section">
        <h3>Ближайшее действие</h3>
        <dl className="detail-definition-list">
          <div><dt>Действие</dt><dd>{task.title}</dd></div>
          <div><dt>Дедлайн</dt><dd>{formatLongDate(task.dueAt)}</dd></div>
          <div>
            <dt>Приоритет</dt>
            <dd className="detail-priority">
              <strong>{task.priorityScore} / 100</strong>
              <span className="priority-track"><span style={{ width: `${task.priorityScore}%` }} /></span>
            </dd>
          </div>
        </dl>
        <div className="detail-reasons">
          {task.priorityReasons.map((reason) => <span key={reason.code}>{reason.label}</span>)}
        </div>
      </section>
      ...
      <section className="detail-section">
        <div className="detail-section-heading">
          <h3>Контакты</h3>
          <button className="detail-link-button" type="button" onClick={() => onAddContact?.(task)} disabled={!onAddContact} title={!onAddContact ? "Только чтение" : undefined}>
          ...
          <div className="contact-list">
            {task.contacts.map((contact) => (
              <article key={contact.id}>
                ...
                <span>{contact.email ?? contact.phone ?? contact.messenger ?? "Канал не указан"}</span>
              </article>
            ))}
          </div>
      ...
      <div className="detail-actions">
        <button className="button button-primary" type="button" onClick={() => onComplete?.(task)} disabled={!onComplete} title={!onComplete ? "Только чтение" : undefined}>
          <Check size={17} aria-hidden="true" /> Выполнить
        </button>
        <button className="button button-secondary" type="button" onClick={() => onReschedule?.(task)} disabled={!onReschedule} title={!onReschedule ? "Только чтение" : undefined}>
          <Clock3 size={17} aria-hidden="true" /> Перенести
        </button>
        <button className="button button-secondary" type="button" onClick={() => onChangeStage?.(task)} disabled={!onChangeStage} title={!onChangeStage ? "Только чтение" : undefined}>
...
function groupLabel(group: TodayAction["group"]) {
  return ({ critical: "Критично", today: "Сегодня", later: "Можно позже", waiting: "Ожидание" })[group];
}
