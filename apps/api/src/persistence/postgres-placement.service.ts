  ...
  async register(input: unknown, idempotencyKey: string): Promise<PlacementView> {
    ...
    return this.prisma.$transaction(
      async (transaction) => {
        ...
        if (duplicate) {
          throw new DomainRuleError("EMB-001", "Такое размещение уже зарегистрировано", {
        ...
        await createOutboxEvent(transaction, {
          ...
          payload: {
            ...
            changedFields: Object.keys(command).filter(
              (field) => field !== "version" && field !== "reason",
            ),
...
function resolvePlacementUpdate(
  ...
  return {
    ...
    businessStatus,
    launchedAt,
    nextCheckAt: businessStatus === "active" ? (shouldCheckNow ? now : current.nextCheckAt) : null,
  };
}
