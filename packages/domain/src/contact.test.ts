...
describe("parseChangeContactStatusCommand", () => {
  it("requires a positive version and an explicit reason", () => {
    expect(parseChangeContactStatusCommand({ version: 3, reason: " Контакт устарел " })).toEqual({
      version: 3,
      reason: "Контакт устарел",
    });
    expect(() => parseChangeContactStatusCommand({ version: 3, reason: "" })).toThrowError(
      DomainRuleError,
    );
  });
});
...
describe("parseLinkContactCommand", () => {
  it("normalizes the organization-specific role", () => {
    expect(parseLinkContactCommand({ role: " Технический эксперт ", department: " ИТ " })).toEqual({
  ...
  it("requires a role for the organization link", () => {
    expect(() => parseLinkContactCommand({ department: "ИТ" })).toThrowError(DomainRuleError);
  });
});
...
describe("parseMergeContactCommand", () => {
  ...
  it("requires an explicit merge reason", () => {
    expect(() => parseMergeContactCommand({ targetContactId: "contact-target" })).toThrowError(
      DomainRuleError,
    );
  });
});
